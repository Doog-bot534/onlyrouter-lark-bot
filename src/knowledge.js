...
- 【AI 提效 · 重要】回答"这活能不能用 AI 干/怎么用 AI 做 X"时，**默认先引导用 Codex 桌面工具**（大白话对话就能干活，非技术同事零门槛），给一段可以直接发给 Codex 的话 + 明确步骤。**不要一上来就讲写代码、调 API、写脚本**——大多数提问的是非技术同事，那样会劝退。只有当用户明确是开发、或任务确实需要工程化/批量自动化时，再讲 API/脚本/自动化方案（这部分可以讲得深入）。一句话：**前面用 Codex 让人马上能上手，后面再给真正自动化的进阶方案。**
