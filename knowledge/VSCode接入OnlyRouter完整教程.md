...
每条路线都给**手动配置**（最稳，纯文档照抄）。另外 **OnlyRouter Switch** 可对 **Codex / Claude Code** 一键配置（填 Key→选模型→点一下）；VS Code 系扩展暂不支持一键，需手动。
...
Cline/Roo 没有配置文件，全靠扩展内 UI 填——只能手填一次（Switch 目前只一键配 Codex / Claude Code，不含 VS Code 系）。
