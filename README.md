...
6. 左侧 **权限管理**，搜索并开通下面这些权限：

   | 权限 code | 作用 | 必需 |
   |-----------|------|------|
   | `im:message` | 收发单聊/群消息（基础） | ✅ 必需 |
   | `im:message.group_at_msg` | 接收群里 @ 机器人的消息 | ✅ 必需 |
   | `im:message.group_at_msg:readonly` | 读取被 @ 消息的内容 | ✅ 必需 |
   | `im:message.p2p_msg` | 接收单聊发来的消息 | 建议（想支持单聊问答就开） |
   | `im:message:send_as_bot` | 以机器人身份主动发消息 | ✅ 必需 |

   > 不同版本的 Lark 后台权限名可能略有差异，认准「**接收群聊中@机器人消息**」「**获取与发送单聊、群组消息**」这两类即可。搜不到某个 code 就按中文名找。
